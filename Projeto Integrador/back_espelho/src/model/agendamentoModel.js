...
const agendamentoModel = {
 ...
 alunos.id_aluno AS id_aluno,
    ...
    agendamentos.id_agendamento AS id_agendamento
FROM 
    agendamentos
INNER JOIN 
...
ORDER BY 
            agendamentos.dia_agendamento DESC
    `,[id.id_profissional])
    return rows;
  },
...
ORDER BY 
            agendamentos.id_agendamento DESC
    `,[id.id_aluno])
    return rows;
  },
  ...
  updateStatusModelConfrim: async (id, novoStatus) => {
    ...
    // Retorna o id do agendamento atualizado
    return { id_agendamento: id, status_agendamento: novoStatus };
  },

  updateStatusModelCancel: async (id, novoStatus) => {
    // Verifica se o status é válido
